-- Registration window timing columns (replaces startAt/endAt) and fee stage rename.
-- Must run after init and add_registration_stages.

ALTER TABLE `RegistrationWindow`
    ADD COLUMN `studentRegistrationOpenAt` DATETIME(3) NULL,
    ADD COLUMN `studentRegistrationCloseAt` DATETIME(3) NULL,
    ADD COLUMN `registrationCloseAt` DATETIME(3) NULL;

UPDATE `RegistrationWindow`
SET
    `studentRegistrationOpenAt` = `startAt`,
    `studentRegistrationCloseAt` = `endAt`,
    `registrationCloseAt` = `endAt`
WHERE `studentRegistrationOpenAt` IS NULL;

ALTER TABLE `RegistrationWindow`
    MODIFY `studentRegistrationOpenAt` DATETIME(3) NOT NULL,
    MODIFY `studentRegistrationCloseAt` DATETIME(3) NOT NULL,
    MODIFY `registrationCloseAt` DATETIME(3) NOT NULL;

DROP INDEX `RegistrationWindow_startAt_endAt_idx` ON `RegistrationWindow`;
