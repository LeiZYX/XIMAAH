  ...
  const lines = [
    ...
    ...rows.map((row) =>
      [
        ...
        row.examSeriesName,
        row.subjectCount,
        row.registrationFeeGbp ?? "",
        row.amountDueGbp,
        row.paymentStatus,
        row.statementStatus,
        row.statementNo ?? "",
        row.generatedAt ?? "",
        row.systemMessages.join("; "),
      ]
        .map(escapeCsv)
        .join(","),
    ),
...
export function exportSummaryXlsx(rows: FeeSummaryRow[]): Buffer {
  const data = rows.map((row) => ({
    ...
    "Registration Fee GBP": row.registrationFeeGbp ?? "",
    "Amount Due GBP": row.amountDueGbp,
    "Payment Status": row.paymentStatus,
    "Statement Status": row.statementStatus,
    "Statement No.": row.statementNo ?? "",
    "Generated At": row.generatedAt ?? "",
    "System Messages": row.systemMessages.join("; "),
  }));
