...
import { reactivateCandidate } from "@/lib/candidates/lifecycle";
...
export async function POST(
  ...
  try {
    await reactivateCandidate(id, auth.user.id, data?.reason);
    const candidate = await getCandidateById(id);
    return NextResponse.json(candidate);
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : "Reactivate failed", 500);
  }
}
